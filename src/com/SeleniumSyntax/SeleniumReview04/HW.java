...
//        once element becomes enable find it and send keys
        ...
        //locate the text "it's disable to make sure the textbox is disable
        WebElement disabledText= driver.findElement(By.xpath("//p[text()=\"It's disabled!\"]"));
        String textDisabled=disabledText.getText();
        System.out.println(textDisabled);

        //check if the text boc is actually disabled now
        if(!txtBox.isEnabled()){
